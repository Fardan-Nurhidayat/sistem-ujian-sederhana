    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Kelola daftar mata pelajaran, kode, KKM, dan guru pengampu.</p>
            </div>

            <a href="{{ route('mata-pelajaran.create') }}">
                <x-button>Tambah Mata Pelajaran</x-button>
            </a>
        </div>
    </x-slot>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $mataPelajaran->nama }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $mataPelajaran->kode }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $mataPelajaran->kkm }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $mataPelajaran->guru?->name ?? '-' }}</td>
                                        ...
                                        <div x-show="open" class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0" style="display: none;">
                                            ...
                                            <div x-show="open" class="mb-6 bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full sm:max-w-lg sm:mx-auto" x-trap.inert.noscroll="open">
                                                ...
                                                <div class="flex flex-row justify-end px-6 py-4 bg-gray-100 text-end gap-3">
                                                    ...
                                                    <form method="POST" action="{{ route('mata-pelajaran.destroy', $mataPelajaran) }}">
                                                        @csrf
                                                        @method('DELETE')
